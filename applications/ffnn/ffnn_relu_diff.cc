...
bbts::ffnn_relu_diff::ffnn_relu_diff() {

  // set the names
  impl_name = "ffnn_relu_diff_cpu";
  ...
  // set the function that actually performs the relu
  fn = &ffnn_relu_diff::apply_relu;
}

// TODO: how to get complexity hint for relu
size_t bbts::ffnn_relu_diff::get_complexity_hint(
...
void bbts::ffnn_relu_diff::get_out_meta(
    const bbts::ud_impl_t::tensor_params_t &params,
    const bbts::ud_impl_t::meta_args_t &_in,
...
void bbts::ffnn_relu_diff::apply_relu(const bbts::ud_impl_t::tensor_params_t &params,
...
/************************** switching the operation calculating here to collecting linearge ******************/
  ...
  for (auto row = 0; row < m_a.num_rows; ++row) {
    for (auto col = 0; col < m_a.num_cols; ++col) {
      ...
      out_data[row * m_a.num_cols + col] = tmp_a > 0 ? 1.0f : 0;
    }
  }
...
void bbts::ffnn_relu_diff::sigmoid(ffnn_dense_t &_out) {
...
void bbts::ffnn_relu_diff::relu(ffnn_dense_t &_out) {
