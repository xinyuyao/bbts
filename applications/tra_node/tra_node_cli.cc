...
static bbts::tid_t temp_tid = 0; // tensor_id
...
// ex. ("R" -> (("0 0" -> 0), ("0 1" -> 1), ("1 0" -> 2), ("1 1" -> 3)), {S,J,A})
//     ("A" -> (("0 0" -> 7), ("0 1" -> 5), ("1 0" -> 5), ("1 1" -> 4)), {S,J})
//     ("J" -> (("0 0" -> 7), ("0 1" -> 5), ("1 0" -> 5), ("1 1" -> 4)), {S})
//     ("S" -> (("0 0" -> 7), ("0 1" -> 5), ("1 0" -> 5), ("1 1" -> 4)), {})
std::map<std::string, std::pair<std::map<std::string, bbts::tid_t>, std::vector<std::string>>> stored_R;
...
bool generate_binary_file(const unsigned row_num, const unsigned col_num, const std::string &file){
  ...
  for(int i = 0;i < row_num * col_num;i++){
    if(i % 2 == 0) data[i] = 0.1;
    else if(i % 3 == 0) data[i] = 0.9;
    else if (i % 7 == 0) data[i] = -3.1;
    else data[i] = - 0.5;
  }
...
void create_id_table(std::ostream &out, bbts::node_t &node, const std::string &db, std::map<std::string, bbts::tid_t> id_map, const std::string &table_name){
  ...
  std::string sql_str;
  std::map< std::string, bbts::tid_t>::iterator it = id_map.begin();
  bbts::tid_t max_tid;
  sql_str += "INSERT INTO " + table_name + " (TOS_ID, TRA_ID) VALUES\n";
  while(it != id_map.end()){
    sql_str += ("(" + std::to_string(it->second) + ", \'" + it->first + "\')");
    max_tid = it->second > current_tid? it->second : current_tid;
    it++;
    if(it != id_map.end()) sql_str += ",\n";
    else sql_str += ";\n";
  }
...
void create_udf_table(std::ostream &out, bbts::node_t &node, const std::string &db){
  ...
  std::string record;
  std::string sql_str;
  std::unordered_map<std::string, std::tuple<bbts::ud_id_t, bool, bool, size_t, size_t>>::iterator udfs_info_it = udfs_info.begin();
  sql_str += "INSERT INTO KERNEL_FUNC (KERNEL_NAME, KERNEL_ID, IS_ASS, IS_COMM, NUM_IN, NUM_OUT) VALUES \n";
  while(udfs_info_it != udfs_info.end()){
    record = "\'";
    ...
    sql_str += ("(" + record + ")");
    udfs_info_it++;
    if(udfs_info_it != udfs_info.end()) sql_str += ",\n";
    else sql_str += ";\n";
  }
...
auto create_tensors(std::ostream &out, bbts::node_t &node, const std::string &file, const std::string split_by_size_or_num, const int32_t row, const int32_t col, const std::string &tensor_type, const std::string &table_name, const std::string &db){
  ...
  std::map<std::string, bbts::tid_t> id_map;
  out << "data shape: ( " << arr.shape[0] << " , " << arr.shape[1]<< " )\n";


  // out << "Start to generate TRA for data file.\n";
  ...
  int32_t sub_row_size = row_size/row_split;
  int32_t sub_col_size = col_size/col_split;
  // out << "sub_row_size: " << sub_row_size << ", sub_col_size: " << sub_col_size << "\n";
  ...
  for(int r = 0; r < row_split; r++){
    for(int c = 0; c < col_split; c++){
      ...
      for(int sub_r = 0; sub_r < sub_row_size; sub_r++){
        std::vector<double> list;
        for(int sub_c = 0; sub_c < sub_col_size; sub_c++){
          list.push_back(data[sub_r * col_size + sub_c + sub_col_size*c + sub_row_size*col_size*r]);
          // out << "(" << r << "," << c << "," << sub_r << "," << sub_c << "): " << sub_r * col_size + sub_c + sub_col_size*c + sub_row_size*col_size*r <<":" << data[sub_r * col_size + sub_c + sub_col_size*c + sub_row_size*col_size*r] << "\n";
        }
  ...
  std::vector<std::string> input_relation_list;
  stored_R.insert(std::pair(table_name, make_pair(id_map,input_relation_list)));
...
bool check_input_output_size_for_kernel_func(std::ostream &out,bbts::node_t &node, const char* db_char, 
...
void generate_matrix_commands(std::ostream &out, bbts::node_t &node,
  ...
  bool check_kernel_func = check_input_output_size_for_kernel_func(out,node, db_char, "uniform", funs[0].input_types.size(), funs[0].output_types.size());
  if(!check_kernel_func) return;
...
void generate_tra_apply_op_commands(std::ostream &out, bbts::node_t &node,
  ...
  // Assume all udf id is smaller than 1000
...
void generate_tra_reduce_op_commands(std::ostream &out, bbts::node_t &node,
                          const std::string &kernel_name, 
                          std::map<std::string, bbts::tid_t> &output_mapping, 
                          std::map<std::string, std::vector<bbts::tid_t>> &input_mapping,
                          const std::string &file_path,
  ...
  for(input_it = input_mapping.begin(); input_it != input_mapping.end(); input_it++){
    commands.push_back(
        bbts::abstract_command_t{.ud_id = udf_id,
                                  .type = bbts::abstract_command_type_t::REDUCE,
                                  .input_tids = input_it->second,
                                  .output_tids = {output_mapping.find(input_it->first)->second},
                                  .params = {}});
    // check_input_output_size_for_kernel_func(out,node, db_char, kernel_name, commands[commands.size() - 1].input_tids.size(), 
...
void generate_aggregation_tra(std::ostream &out, bbts::node_t &node, std::map<std::string, bbts::tid_t> &output_mapping, 
  ...
  std::map<std::string, std::pair<std::map<std::string, bbts::tid_t>, std::vector<std::string>>>::iterator stored_R_it = stored_R.find(tr_name);
  if(stored_R_it == stored_R.end()) relation_exist_flag = false;

  if(!relation_exist_flag){
    return; 
  }

  std::map<std::string, bbts::tid_t> id_map = stored_R_it->second.first;
  ...
  for(map_it = id_map.begin(); map_it!= id_map.end(); map_it++){
    ...
    if(it == input_mapping.end()){
      ...
      output_mapping.insert(std::pair(agg_key, temp_tid++));
    }
    else{
  ...
  stored_R.insert(std::pair(stored_tr_name, make_pair(output_mapping, input_relation_list)));
...
void generate_join_tra(std::ostream &out, bbts::node_t &node, std::map<std::string, bbts::tid_t> &output_mapping, 
  ...
  std::map<std::string, std::pair<std::map<std::string, bbts::tid_t>, std::vector<std::string>>>::iterator stored_R_it_l = stored_R.find(tr_name_l);
  std::map<std::string, std::pair<std::map<std::string, bbts::tid_t>, std::vector<std::string>>>::iterator stored_R_it_r = stored_R.find(tr_name_r);
  if(stored_R_it_l == stored_R.end() || stored_R_it_r == stored_R.end()) relation_exist_flag = false;
  // return if relation does not exist
  if(!relation_exist_flag){
    return;
  }

  std::map<std::string, bbts::tid_t> id_map_l = stored_R_it_l->second.first;
  std::map<std::string, bbts::tid_t> id_map_r = stored_R_it_r->second.first;
  // Generate Aggregate commands pairs based on aggregate dimension list
  ...
  for(map_it1 = id_map_l.begin(); map_it1!= id_map_l.end(); map_it1++){
    for(map_it2 = id_map_r.begin(); map_it2!= id_map_r.end(); map_it2++){
      // out << "****************  checking " << map_it1->second << " & " << map_it2->second << "\n";
      ...
      for(int i = 0; i < dimension_list_l.size(); i++){
        // out << "left: " << split_key_l[stoi(dimension_list_l[i])] << " | right: " << split_key_r[stoi(dimension_list_r[i])] << "\n";
      ...
      if(join_key_match){
        ...
        output_mapping.insert(std::pair(join_key, temp_tid++));
  ...
  std::vector<std::string> input_relation_list;
  std::vector<std::string> input_relation_list_l = stored_R.find(tr_name_l)->second.second;
  std::vector<std::string> input_relation_list_r = stored_R.find(tr_name_r)->second.second;
  if(input_relation_list_l.size() == 0) input_relation_list = input_relation_list_l;
  else if(input_relation_list_r.size() == 0) input_relation_list = input_relation_list_r;
  else{
    std::merge(input_relation_list_l.begin(), input_relation_list_l.end(), input_relation_list_r.begin(), 
             input_relation_list_r.end(), input_relation_list.begin());
  }
  
  input_relation_list.push_back(tr_name_l);
  input_relation_list.push_back(tr_name_r);
  stored_R.insert(std::pair(stored_tr_name, make_pair(output_mapping, input_relation_list)));
}
...
void handle_math_op_for_rekey(std::ostream &out, bbts::node_t &node, const std::string &tr_name, const std::string &op, 
  ...
  if(new_id_map.empty()){
    ...
    old_id_map = stored_R_it->second.first;
  }
  else{
...
void handle_collective_op_for_rekey(std::ostream &out, bbts::node_t &node, const std::string &tr_name, const std::string &op, 
  ...
  std::map<std::string, bbts::tid_t> old_id_map = stored_R_it->second.first;
...
void change_dim_for_rekey(std::ostream &out, bbts::node_t &node, const std::string &tr_name, const std::string &dim_list, 
                          const std::string db, std::map<std::string, bbts::tid_t> id_map){
  int max_tid;
  bool relation_exist_flag = true;
  std::map<std::string, bbts::tid_t> old_id_map = get_id_map_from_sqlite(out, node, db.c_str(), max_tid, tr_name, relation_exist_flag);
  ...
  std::string type;
  std::vector<std::string> num_or_dim_list;

  math_op_prep(out, dim_list, type, num_or_dim_list);
  if(type != "dimension"){
    out << "Input has to be dimensions.\n";
    return;
  }
  ...
  for(id_map_it = old_id_map.begin(); id_map_it != old_id_map.end(); id_map_it++){
    ...
    for(int i = 0; i < num_or_dim_list.size(); i++){
      new_tra_id.push_back(tra_id[std::stoi(num_or_dim_list[i])]);
    }
...
void drop_dimensions_from_relation(std::ostream &out, bbts::node_t &node, const std::string &tr_name, const std::string &drop_dimensions, const std::string &db){
  ...
  if(drop_dimensions_type != "dimension"){
...
void delete_all_table(std::ostream &out, bbts::node_t &node, const std::string &db){
  ...
  sqlite3_stmt * stmt;
  sqlite3 *sql_db;
  std::vector<std::string> all_table_info;
  ...
  if(sqlite3_open(db_name, &sql_db) == SQLITE_OK){
    ...
    while((rc = sqlite3_step(stmt)) == SQLITE_ROW){
      std::string table_info = std::string(reinterpret_cast< const char* >(sqlite3_column_text(stmt, 1)));
      all_table_info.push_back(table_info);
      
    }
    

    for(auto t: all_table_info){
      std::string sub_sql = "DROP TABLE " + t + ";\n";
      const char* sub_sql_char = sub_sql.c_str();
      execute_command(sub_sql_char, db_name, 0);
      out << t << " is deleted \n";
    }
    out << "\n";
...
void prompt(bbts::node_t &node) {
  ...
  },"Create sqlite table based on provided db name (eg.\"tra.db\"). Usage : generate sqlite_db <db_name>\n");
  ...
  rootMenu->Insert("rekey",[&](std::ostream &out, const std::string &tr_name, const std::string &dim_list) {
    const std::string &db = "tra.db";
    std::map<std::string, bbts::tid_t> id_map;
    change_dim_for_rekey(out, node, tr_name, dim_list, db, id_map);
  ...
  rootMenu->Insert("stored_R",[&](std::ostream &out) {

    std::map<std::string, std::pair<std::map<std::string, bbts::tid_t>, std::vector<std::string>>>::iterator it;
    for(it = stored_R.begin(); it != stored_R.end(); it++){

      std::map<std::string, bbts::tid_t> id_map = it->second.first;
      std::vector<std::string> relation_list = it->second.second;

      out << it->first << " ===> ";
      std::map<std::string, bbts::tid_t>::iterator id_map_it;
      for(id_map_it = id_map.begin(); id_map_it != id_map.end(); id_map_it++){
        out << "( " << id_map_it -> first << " , " << id_map_it -> second << " )  ";
      }
      out << "\n ===> ";
  ...
  rootMenu->Insert("delete_all_tables",[&](std::ostream &out){
    const std::string &db = "tra.db";
