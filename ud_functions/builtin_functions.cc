...
void bbts::dense_matrix_add_t::add(const bbts::ud_impl_t::tensor_params_t &_in, bbts::ud_impl_t::tensor_params_t &_out) {
  ...
  for (auto row = 0; row < m_a.num_rows; ++row) {
    for (auto col = 0; col < m_a.num_cols; ++col) {
      out.data()[row * m_a.num_cols + col] += b.data()[row * m_a.num_cols + col];
    }
  }
